...
impl ActiveSession {
    pub fn open_new() -> std::io::Result<Self> {
        ...
        let s = Self {
            inner: Mutex::new(Inner { writer, session_id: id, clock, wall_epoch_ns, seq: 0 }),
        };
        s.append_internal(Source::System, None, Payload::SessionStarted { wall_unix_ns: wall_epoch_ns })?;
        Ok(s)
    }
    ...
    pub fn append(&self, source: Source, pid: Option<u32>, payload: Payload) -> std::io::Result<Event> {
        self.append_internal(source, pid, payload)
    }

    fn append_internal(&self, source: Source, pid: Option<u32>, payload: Payload) -> std::io::Result<Event> {
        ...
        let ev = Event {
            ...
            source, pid, seq: inner.seq, payload,
        };
        inner.writer.write_event(&ev)
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        Ok(ev)
    }

    pub fn finalize(self, exit_code: Option<i32>, reason: &str) -> std::io::Result<()> {
        let _ = self.append(Source::System, None, Payload::SessionEnded {
...
mod tests {
    use super::*;
    use smeltr_core::reader::{read_events, read_metadata, list_sessions};
    ...
    fn session_lifecycle_appends_start_and_end() {
        ...
        s.append(Source::Mark, None, Payload::Mark { label: "hi".into() }).unwrap();

...
- `SMELTR_SCOPE_MEM_SAMPLE` — on by default. Every `smeltr.scope("...")` enter and exit
  reads MLX's Metal allocation synchronously (`get_active_memory() + get_cache_memory()` —
  the MLX share of `MTLDevice.currentAllocatedSize`, which the hook's samples report — plus
  `device_info()["max_recommended_working_set_size"]` as the budget) and emits a
  `MetalDeviceMemSample` event with `at_event="scope_enter"|"scope_exit"`. Active memory
  alone understated scope peaks (#243). Set to `0` to opt out (one syscall
...
- `SMELTR_HOOK_GPUTRACE_CBS=<n>` / `SMELTR_HOOK_GPUTRACE_PATH=<path>` — set
  ...
  The traced run completes normally: verified on a real LTX-2 distilled run
  at 72.2 s against 61-70 s uncaptured. An earlier revision of this note
  claimed the run aborts — that was a deadlock of our own making (stopping
  the capture from inside the `commit` path), not an MLX/Metal
  incompatibility, and it is fixed.
...
- `SMELTR_FOOTPRINT_PERIOD_MS=<n>` — sampling cadence for `phys_footprint`
  ...
  **daemon** process — set it in the daemon's environment, not on the
  `smeltr record` invocation (measured: var on `record` gave 2 samples over a
  3 s run, i.e. the 2 s default was unaffected; var on the daemon gave 15).
  This differs from `SMELTR_SESSION_INDEX`, which the record client forwards
  per-session to the daemon (#188). The probe calls `proc_pid_rusage` per pid
  in the tree plus one `proc_listallpids` enumeration per tick — measured
  cost on this machine (~480 visible processes): `sample_tree` 562 µs/tick
  (`list_processes` ≈ 546 µs, `read_footprint` ≈ 588 ns), i.e. ~0.03 % of one
  core at the 2 s default — negligible, cadence left at 2 s. Set it lower to
  tighten the curve ahead of a jetsam death, higher on multi-hour runs.
